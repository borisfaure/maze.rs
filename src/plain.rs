...
pub fn draw_cell_plain_gif<T: Rendering>(renderer: &T, width: usize,
                                         buffer: &mut Vec<u8>,
                                         c: &Coord, color_idx: u8) {
    let tile_size = renderer.tile_size();
    for i in 0..tile_size {
        for j in 0..tile_size {
            buffer[(c.x * tile_size + i) +
                   (c.y * tile_size + j) * width] = color_idx;
        }
    }
}
...
impl Rendering for RendererPlain {
    ...
    fn draw_cell_gif(&self, _maze: &Maze, img_geom: &super::Geometry,
                     buffer: &mut Vec<u8>, c: &Coord, cell_kind: CellKind) {
        match cell_kind {
            CellKind::PathKind(f) => {
                if self.path_color_start != self.path_color_end
                    && !f.is_nan() && f < 1f64 && f >= 0f64 {
